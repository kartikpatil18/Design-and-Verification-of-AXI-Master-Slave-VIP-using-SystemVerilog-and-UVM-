class axi_master_driver extends uvm_driver#(axi_tx);
	...
	typedef struct{
		...
		function void build_phase(uvm_phase phase);
			...
			uvm_config_db#(virtual axi_interface)::get(this,"","v_intf",v_m_intf);//pointing from virtual interface to virtual interface
		...
		task write_address_channel(axi_tx req);
			v_m_intf.awaddr<=req.awaddr;
			v_m_intf.awid<=req.awid;
			v_m_intf.awvalid<=req.awvalid;
			v_m_intf.awlen<=req.awlen;
			v_m_intf.awsize<=req.awsize;
			v_m_intf.awburst<=req.awburst;
			v_m_intf.awprot<=req.awprot; 
			v_m_intf.awlock<=req.awlock;
			v_m_intf.awcache<=req.awcache;
		...
		task write_data_channel(axi_tx req);
			...
			for (int i=0; i<=v_m_intf.awlen; i++)begin //number of beats
				...
				//slave, once wready came then only master will generate
		...
		task read_address_channel(axi_tx req);
			...
			v_m_intf.araddr<=req.araddr;
			v_m_intf.arid<=req.arid;
			v_m_intf.arvalid<=req.arvalid;
			v_m_intf.arlen<=req.arlen;
			v_m_intf.arsize<=req.arsize;
			v_m_intf.arburst<=req.arburst;
			v_m_intf.arprot<=req.arprot;
			v_m_intf.arlock<=req.arlock;
			v_m_intf.arcache<=req.arcache;
		...
		task strobe_genrate(axi_tx req);//unaligned(first best all bytes are not active and remaining beats all bytes are active), narrow transfer all beatss aall bytes not active(every beat some bytes and some bytes are inactive)
			  ...
			  bytes_in_each_beat = (2**req.awsize);
			  //aligned address or unaligned address, if address is aligned then
			  //reminder 0, if address unaligned remainder not equals to 0
			  address_reminder = req.awaddr % total_data_width_in_bytes;
		      //convert to aligned address   
			  unaligned_to_aligned_address = req.awaddr - (req.awaddr % bytes_in_each_beat);//not narrow, normal transfer conversion
			  //below 2 formulas mainly used to achive unaligned types transaction 
		      //address diffrence number of byte location
			  address_diff = req.awaddr - unaligned_to_aligned_address; 
		      //narrow transfer 
			  narrow_transfer = address_reminder - address_diff;
		
			  req.wstrb=0;
		
			  //aligned transaction 
			  if(req.awaddr % bytes_in_each_beat ==0)begin 
				  for(int j=0; j<bytes_in_each_beat; j++)begin 
	...
	task overlapping_write_data();
		...
		forever begin
			...
			for (int i=0; i<=req.awlen; i++)begin //number of beats
				v_m_intf.wdata<=req.wdata.pop_back();
				v_m_intf.wvalid<=req.wvalid;
				v_m_intf.wid<=req.wid;
				//wstrb
				strobe_genrate(req);//narrow transfer, normal transfer 
	...
	task send_interleave_data();
		...
		selected_id=active_idx[$urandom_range(0, active_idx.size() -1)];  
		...
		if(selected_tracker.beat_sent == (selected_tracker.total_beats-1))begin  
			v_m_intf.wlast<=1;
