	...
	task run_phase(uvm_phase phase);
		...
		forever begin
		       ...
		       if(tx.wvalid==1 && tx.wready==1)begin 
				    wdata_burst_width_in_bytes = ($size(wr_data)/8); 
				    count=0;
			    ...
			    if(tx.arvalid==1)begin 
				 ...
				 if(tx.rvalid==1 && tx.rready==1)begin
		            ...
		            for(int i=0; i< range; i++)begin 
		                actual_data[rd_tx[tx.rid].araddr+count]=rd_data[i*8 +:8];//reading from memeory
		             ...
		             rd_tx[tx.rid].araddr= rd_tx[tx.rid].araddr - (rd_tx[tx.rid].araddr % 2**rd_tx[tx.rid].arsize);
		             rd_tx[tx.rid].araddr= rd_tx[tx.rid].araddr + 2**rd_tx[tx.rid].arsize;
